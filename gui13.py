...
##curso.execute("DELETE FROM evento WHERE idEv")

#BD.text_factory = str
#curso.execute("SELECT last_insert_rowid()")

##curso.execute(" CREATE TABLE evento (idEv INTEGER PRIMARY KEY AUTOINCREMENT, d0 DATE, t0 TIME,df DATE, tf TIME, tomada INT, pot FLOAT,sigmaPot DOUBLE)")
...
class Texto:
        def __init__(self, master=None):
          ...
          self.hrLabel = Label(self.quintoContainer,text="Horarios:", font=self.fontePadrao)
...
def plotPot():
        ...
        curso.execute('''SELECT sigmaPot FROM evento WHERE tomada = 1 AND (sigmaPot IS NOT NULL OR trim(sigmaPot) = '')''')
        isso1=curso.fetchall()
        curso.execute('''SELECT sigmaPot FROM evento WHERE tomada = 2 AND (sigmaPot IS NOT NULL OR trim(sigmaPot) = '')''')
        isso2=curso.fetchall()
        curso.execute('''SELECT sigmaPot FROM evento WHERE tomada = 3 AND (sigmaPot IS NOT NULL OR trim(sigmaPot) = '')''')
        isso3=curso.fetchall()
        curso.execute('''SELECT sigmaPot FROM evento WHERE tomada = 4 AND (sigmaPot IS NOT NULL OR trim(sigmaPot) = '')''')
        isso4=curso.fetchall()
        ...
        for i in range(0,len(isso1)):
                soma1+=float(isso1[i][0])

        for i in range(0,len(isso2)):
                soma2+=float(isso2[i][0])

        for i in range(0,len(isso3)):
                soma3+=float(isso3[i][0])

        for i in range(0,len(isso4)):
                soma4+=float(isso4[i][0])
                
        curso.execute("SELECT count(*) FROM evento WHERE tomada = 1 ")
        numum = curso.fetchall()
        curso.execute("SELECT count(*) FROM evento WHERE tomada = 2 ")
        numdo = curso.fetchall()
        curso.execute("SELECT count(*) FROM evento WHERE tomada = 3 ")
        numtr = curso.fetchall()
        curso.execute("SELECT count(*) FROM evento WHERE tomada = 4 ")
        numua = curso.fetchall()

        print(soma1)
        print(soma2)
                
        GastoTomada1.config(text="Potencia gasta na T1 neste periodo: " + str(int(soma1/3600000))+"KWh R$"+str(int((soma1/3600000)*0.5))+" #"+str(numum[0][0]), font = myFont)
        GastoTomada2.config(text="Potencia gasta na T2 neste periodo: " + str(int(soma2/3600000))+"KWh R$"+str(int((soma2/3600000)*0.5))+" #"+str(numdo[0][0]), font = myFont)
        GastoTomada3.config(text="Potencia gasta na T3 neste periodo: " + str(int(soma3/3600000))+"KWh R$"+str(int((soma3/3600000)*0.5))+" #"+str(numtr[0][0]), font = myFont)
        GastoTomada4.config(text="Potencia gasta na T4 neste periodo: " + str(int(soma4/3600000))+"KWh R$"+str(int((soma4/3600000)*0.5))+" #"+str(numua[0][0]), font = myFont)
...
GastoTomada1 = Label(win, text="Potencia gasta na T1 neste periodo: " + str(int(soma1/360000))+"KVh"+str(int((soma1/360000)*0.5)), font = myFont)
GastoTomada2 = Label(win, text="Potencia gasta na T2 neste periodo: " + str(int(soma2/360000))+"KVh"+str(int((soma2/360000)*0.5)), font = myFont)
GastoTomada3 = Label(win, text="Potencia gasta na T3 neste periodo: " + str(int(soma3/360000))+"KVh"+str(int((soma3/360000)*0.5)), font = myFont)
GastoTomada4 = Label(win, text="Potencia gasta na T4 neste periodo: " + str(int(soma4/360000))+"KVh"+str(int((soma4/360000)*0.5)), font = myFont)
...
GastoTomada4.pack(fill=X,side=BOTTOM)
GastoTomada3.pack(fill=X,side=BOTTOM)
GastoTomada2.pack(fill=X,side=BOTTOM)
GastoTomada1.pack(fill=X,side=BOTTOM)


exitButton = Button(win, text = "Exit", font = myFont, command = exitProgram, height =2 , width = 6,bg = "blue") 
exitButton.place(x=1250, y=25)
