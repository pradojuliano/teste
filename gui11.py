...
def T1d():
        ...
        if ( isso[0][0] == "I"):
                ...
                curso.execute('''UPDATE evento SET sigmaPot = ? WHERE idEv = ?''',( (b-a).total_seconds(),int(isso[0])))
                GPIO.output(12,1)
...
def T2d():
        ...
        if ( isso[0][0] == "I"):
                ...
                a = dt.datetime(int(issod[8:]),int(months[issod[4:-6]]),int(issod[1:-10]),int(issoh[0:-6]),int(issoh[3:-3]),int(issoh[6:]))
                b = dt.datetime(int(strftime('%Y')),int(strftime('%m')),int(strftime('%d')),int(strftime('%H')),int(strftime('%M')),int(strftime('%S')))
                ba = (float((b-a).total_seconds())*(float(akilo[0][0])))
                                                   
                curso.execute('''UPDATE evento SET sigmaPot = ? WHERE idEv = ?''',( ba,int(isso[0]) ))              
                GPIO.output(16,1)
